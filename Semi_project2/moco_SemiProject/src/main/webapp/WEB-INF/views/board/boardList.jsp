...
<body>
    ...
    <hr>
        ...
        <table class="table" id="CategoryT">
            ...
            <c:if test="${ !empty loginUser }">
	            ...
	            <th><button type="button" id="unknown" onclick="location.href='${contextPath}/boardUnKnown.bo'"> 자유 게시판</button></th>
        ...
        <table class="table" id="BoardListT">
            <tr style="height: 50px;">
                <td colspan="6">
                    ...
                    <c:if test="${ boardCategory eq 'FREE' or boardCategory eq 'main' }">
                   		 <input type="button" id="boardAnonymity" value="N" onclick="boardAnonymity();">
                    </c:if>
                    <c:if test="${ boardCategory ne 'FREE' or  boardCategory ne 'main'}">
                   		 <input type="hidden" id = "boardAnonymity" value="N">
                    </c:if>
                </td>
            </tr>
            <tr>
	         	...
	         	<c:forEach items="${ list }" var="b">
		          			<tr>
		          				<c:if test="${ b.boardReport eq 'N' }">
			            			...
			            			<c:if test="${ b.boardAnonymity eq 'Y' }">
			            				<c:if test="${boardAnonymity ne 'N'}">
			            					<td>익명</td>
			            				</c:if>
			            			</c:if>
	...
	<script>
		...
		const searchBoard = () =>{
			...
			const boardAnonymity = document.getElementById('boardAnonymity').value;
			...
			if(boardAnonymity == null){
				location.href='${contextPath}/Search.bo?searchCondition='+searchCondition+"&searchValue=" + searchValue+"&boardCategory=" + boardCategory
			}
